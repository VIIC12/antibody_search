...
logging.getLogger("search_engine").setLevel(
    logging.DEBUG
)  # In case it's imported without src. prefix
...
def main():
    ...
    igblast = st.Page(
        "pages/igblast.py", title="IgBLAST", icon=":material/genetics:"
    )
    statistics = st.Page(
        "pages/statistics.py", title="Statistics", icon=":material/bar_chart:"
    )
    imprint = st.Page(
        "pages/imprint.py", title="Imprint", icon=":material/info:"
    )
