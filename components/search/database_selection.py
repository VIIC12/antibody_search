...
DB_SELECTION_LOCK_MESSAGE = ":material/hourglass: Database selection is temporarily locked while a search or plotting task is running."
...
def render_database_selection(
    db_structure: dict, disabled: bool = False
    ...
    heavy_total_sequences = sum(
        info["sequence_count"] for info in db_structure["Heavy"].values()
    )
    light_total_sequences = sum(
        info["sequence_count"] for info in db_structure["Light"].values()
    )
    ...
    has_inferred_overlay = (
        "Inferred" in db_structure
        and len(db_structure["Inferred"]) > 0
        and any(
            info.get("is_inferred", False)
            for info in db_structure["Inferred"].values()
        )
    )
    ...
    paired_conflict_disabled = (
        heavy_currently_selected or light_currently_selected
    )
    ...
    with col1:
        ...
        if selection_locked:
            ...
            heavy_help = (
                "Inferred data is automatically included when searching Heavy or Light chains."
                if has_inferred_overlay
                else None
            )
                    ...
                    for (subdir, info), col in zip(row_items, sub_cols):
                        with col:
                            ...
                            checkbox_help = (
                                ...
                                if selection_locked
                                else f"Files: {info['parquet_count']} | Sequences: {info['sequence_count']:,}"
                            )
    ...
    with col3:
        ...
        if paired_selected:
            paired_real_entries = [
                (subdir, info)
                for subdir, info in db_structure["Paired"].items()
                if not info.get("is_inferred", False)
            ]
            ...
            if len(paired_real_entries) == 1:
                ...
                real_sequence_total = sum(
                    info["sequence_count"] for _, info in paired_real_entries
                )
                real_label = "Paired"
                real_help_parts = []
                if len(paired_real_entries) > 1:
                    real_help_parts.append(
                        ...
                        + ", ".join(name for name, _ in paired_real_entries)
                    )
    ...
    loadable_databases = [
        db for db in selected_databases if not _is_inferred_path(db)
    ]
    selected_db = (
        loadable_databases[0]
        if loadable_databases
        else (selected_databases[0] if selected_databases else None)
    )
    ...
    effective_selected = st.session_state.get(
        "effective_selected_databases", selected_databases
    )
...
def initialize_database(selected_databases: List[str]) -> bool:
    """
    ...
    overlay_databases = [
        db for db in selected_databases if _is_inferred_path(db)
    ]
    ...
    has_unpaired = any(
        "Heavy" in db or "Light" in db for db in loadable_databases
    )
    has_paired = any(
        "Paired" in db and "Inferred" not in db for db in loadable_databases
    )
    ...
    if need_reinit:
        ...
        for db_path in loadable_databases:
            ...
            if not parquet_files or not metadata_fresh:
                st.error(
                    f"Database files not found or metadata is outdated for {db_path}."
                )
                return False
        ...
        with st.spinner("Loading database..."):
            ...
            effective_selection = list(
                dict.fromkeys(loadable_databases + overlay_databases)
            )
            st.session_state["effective_selected_databases"] = (
                effective_selection
            )
            st.session_state["selected_databases"] = effective_selection
            st.session_state["inferred_overlay_dirs"] = overlay_databases
            st.session_state["inferred_overlay_active"] = bool(
                overlay_databases
            )
    ...
    st.session_state["loadable_databases_active"] = loadable_databases
    st.session_state["overlay_databases_active"] = overlay_databases
    st.session_state["effective_selected_databases"] = list(
        dict.fromkeys(loadable_databases + overlay_databases)
    )
    st.session_state["selected_databases"] = st.session_state[
        "effective_selected_databases"
    ]
    st.session_state["inferred_overlay_dirs"] = overlay_databases
        ...
        try:
            ...
            if metadata_file.exists():
                ...
                if "total_sequences" in metadata_df.columns:
                    total_selected_sequences += metadata_df[
                        "total_sequences"
                    ].sum()
