...
class TestAntibodySearchEngine:
    ...
    def mock_metadata(self, temp_dir):
        ...
        metadata = pd.DataFrame({
            ...
            "file_path": [
                ...
                "parquet/test_paired_data.parquet",
            ],
    ...
    def test_database_selection_unpaired(self, mock_unpaired_data):
        ...
        assert not engine.schema["heavy_chain"]
        assert not engine.schema["light_chain"]
    ...
    def test_database_selection_paired(self, mock_paired_data):
        ...
        assert engine.schema["heavy_chain"]
        assert engine.schema["light_chain"]
    ...
    def test_light_chain_v_gene_search_unpaired(self, mock_light_unpaired_data):
        ...
        results_df, stats_df, stats = engine.search(
            chain_mode="light", light_v="2"
        )
        assert (
            stats["total_hits"] == 2
        )  # Matches both lambda and kappa family 2 genes

        results_df, stats_df, stats = engine.search(
            chain_mode="light", light_v="L2"
        )
        assert stats["total_hits"] == 1  # Lambda-only prefix

        results_df, stats_df, stats = engine.search(
            chain_mode="light", light_v="K2"
        )
        assert stats["total_hits"] == 1  # Kappa-only prefix

        results_df, stats_df, stats = engine.search(
            chain_mode="light", light_v="IGLV2-14*01"
        )
    ...
    def test_light_chain_j_gene_search_unpaired(self, mock_light_unpaired_data):
        ...
        results_df, stats_df, stats = engine.search(
            chain_mode="light", light_j="2"
        )
        assert (
            stats["total_hits"] == 2
        )  # Matches both lambda and kappa family 2 J genes

        results_df, stats_df, stats = engine.search(
            chain_mode="light", light_j="L2"
        )
        assert stats["total_hits"] == 1  # Lambda-only prefix

        results_df, stats_df, stats = engine.search(
            chain_mode="light", light_j="K2"
        )
        assert stats["total_hits"] == 1  # Kappa-only prefix
    ...
    def test_combined_search_unpaired(self, mock_unpaired_data):
        ...
        results_df, stats_df, stats = engine.search(
            chain_mode="heavy", heavy_v="3-23", heavy_cdr3_length=6
        )
        assert stats["total_hits"] == 2  # Two sequences match both criteria

        # Test V gene + D gene + J gene combination
        results_df, stats_df, stats = engine.search(
            chain_mode="heavy", heavy_v="3-23", heavy_d="3-10", heavy_j="J4"
        )
    ...
    def test_combined_heavy_light_search_paired(self, mock_paired_data):
        ...
        results_df, stats_df, stats = engine.search(
            heavy_v="3-23", light_v="1-39"
        )
        assert (
            stats["total_hits"] == 1
        )  # One sequence matches both heavy and light V genes

        # Test heavy + light CDR lengths
        results_df, stats_df, stats = engine.search(
            heavy_cdr3_length=6, light_cdr3_length=5
        )
    ...
    def test_full_results_vs_statistics(self, mock_unpaired_data):
        ...
        results_df, stats_df, stats = engine.search(
            chain_mode="heavy", heavy_v="3-23", full_results=True
        )
    ...
    def test_limit_parameter(self, mock_unpaired_data):
        ...
        # Test with limit
        results_df, stats_df, stats = engine.search(
            chain_mode="heavy", heavy_v="3-23", full_results=True, limit=1
        )
    ...
    def test_metadata_functionality(self, mock_unpaired_data, mock_metadata):
        ...
        if not results_df.empty:
            assert (
                "percentage" in results_df.columns
                or "per_million" in results_df.columns
            )
    ...
    def test_edge_cases_and_error_handling(self, temp_dir):
        ...
        with pytest.raises(
            Exception
        ):  # Should handle invalid parquet gracefully
    ...
    def test_database_info_functionality(self, mock_unpaired_data):
        ...
        assert info["is_memory_db"]  # Default is memory database
    ...
    def test_progress_callback(self, mock_unpaired_data):
        ...
        engine = AntibodySearchEngine(
            data_dir=data_dir, progress_callback=mock_callback
        )
    ...
    def test_memory_vs_file_database(self, mock_unpaired_data, temp_dir):
        ...
        engine_memory = AntibodySearchEngine(
            data_dir=data_dir, db_path=":memory:"
        )
        assert engine_memory.get_database_info()["is_memory_db"]
        engine_memory.close()

        # Test file database
        temp_db = Path(temp_dir) / "test.db"
        engine_file = AntibodySearchEngine(
            data_dir=data_dir, db_path=str(temp_db)
        )
        assert not engine_file.get_database_info()["is_memory_db"]
    ...
    def test_empty_search_results(self, mock_unpaired_data):
        ...
        results_df, stats_df, stats = engine.search(
            chain_mode="heavy", heavy_v="999-999"
        )
    ...
    def test_special_characters_in_motifs(self, mock_unpaired_data):
        ...
        results_df, stats_df, stats = engine.search(
            chain_mode="heavy", heavy_cdr1_motif="AR.S*"
        )
        assert stats["total_hits"] >= 0  # Should not crash

        # Test motifs with wildcards
        results_df, stats_df, stats = engine.search(
            chain_mode="heavy", heavy_cdr1_motif="AR*"
        )
        assert stats["total_hits"] >= 0  # Should not crash

        engine.close()

    def test_search_timeout_constraint(self, mock_unpaired_data):
        """Test search with timeout constraint (20s)."""
        data_dir, mock_data = mock_unpaired_data
        engine = AntibodySearchEngine(data_dir=data_dir)

        def run_search():
            return engine.search(chain_mode="heavy", heavy_cdr1_length=">=2")

        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(run_search)
            try:
                results_df, stats_df, stats = future.result(timeout=20)

                # Verify results
                assert stats["total_hits"] > 0
                # All mock data has cdr1_length 5, which is >= 2
                assert stats["total_hits"] == 5

            except concurrent.futures.TimeoutError:
                pytest.fail("Search timed out after 20 seconds")
            finally:
                engine.close()
