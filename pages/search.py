            ...
            if current_db != selected_db:
                ...
                if parquet_files:
                    if check_metadata_freshness(selected_db):
                        ...
                        # Show reindex button
                        if st.button("🚀 Reindex Database", type="primary", width='stretch'):
                            st.session_state['reindex_requested'] = True
                            st.session_state['reindex_db'] = selected_db
                            st.rerun()
                else:
                    st.button("No Data", disabled=True, width='stretch')
            else:
                ...
                if not check_metadata_freshness(selected_db):
                    ...
                    # Metadata is fresh
                    if st.button("🔄 Reindex Database", help="Click after adding/removing files", width='stretch'):
                        st.session_state['reindex_requested'] = True
                        st.session_state['reindex_db'] = selected_db
                        st.rerun()
        else:
