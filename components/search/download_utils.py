...
def _get_download_path(
    original_filename: str, file_type: str = "zip"
) -> Tuple[str, Path, str]:
    """
...
def _build_where_clause_from_params(
    ...
    conditions = []
    base_where = build_plotting_where_clause(
    ...
    if is_paired:
        ...
        # Unpaired motifs
        cdr_prefix = (
            "light_" if unpaired_chain_type.lower() == "light" else "heavy_"
        )
        ...
        for cdr_num in [1, 2, 3]:
            ...
            motif = search_params.get(motif_key) or search_params.get(
                f"cdr{cdr_num}_motif", ""
            )
            if motif:
                similarity = search_params.get(
                    similarity_key
                ) or search_params.get(f"cdr{cdr_num}_similarity", False)
                mismatches = search_params.get(
                    mismatches_key
                ) or search_params.get(f"cdr{cdr_num}_mismatches", 2)
...
def prepare_stats_download(
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
        ...
        zip_file.writestr(
            "search_parameters.json", metadata_json.encode("utf-8")
        )
...
def _build_copy_select_columns(
    schema_column_names: List[str], is_paired: bool
) -> str:
    """
...
def prepare_full_results_download_background(
    ...
    try:
        ...
        engine = init_search_engine(data_dir=database_paths, db_path=":memory:")
        logger.info(
            "Background full results download: Search engine initialized"
        )
        ...
        token, csv_gz_path, download_url = _get_download_path(
            csv_gz_filename, "csv.gz"
        )
        ...
        logger.info(
            f"Background full results download: Completed. File size: {file_size_bytes} bytes. Sequences: {sequence_count}"
        )
        ...
        logger.error(
            f"Background full results download failed: {str(e)}", exc_info=True
        )
        return {
            "success": False,
            "error": str(e),
...
def prepare_plots_download_background(
    ...
    try:
        ...
        logger.info(
            f"Background plots download: Started. Plots count: {len(plots_data) if plots_data else 0}"
        )
        ...
        if include_raw_data:
            filename = (
                f"abhunter_result_plots_raw_data_{chain_label}_{seed_hash}.zip"
            )
        else:
        ...
        try:
            with zipfile.ZipFile(file_path, "w", zipfile.ZIP_DEFLATED) as zf:
                ...
                metadata_with_plots["includes_raw_plotting_data"] = (
                    include_raw_data
                )
        ...
        logger.error(
            f"Background plots download failed: {str(e)}", exc_info=True
        )
        return {
            "success": False,
            "error": str(e),
...
def prepare_fasta_download_background(
    ...
    try:
        ...
        # Copy search params to avoid modifying original
        search_params_copy = search_params.copy()
        selected_databases_formatted = search_params_copy.pop(
            "selected_databases", []
        )
        ...
        if is_paired:
            ...
            if "sequence_alignment_aa" in all_columns:
                ...
                "sequence_alignment_aa_heavy" in all_columns
                and "sequence_alignment_aa_light" in all_columns
            ):
                seq_columns = [
                    "sequence_alignment_aa_heavy",
                    "sequence_alignment_aa_light",
        ...
        try:
            ...
            if hasattr(engine, "conn"):
                engine.conn.close()
            fasta_files = _csv_to_fasta_files(
                temp_csv_path, is_paired, seq_columns, chain_label
            )
        finally:
        ...
        with zip_fp as zip_file:
            ...
            zip_file.writestr(
                "search_parameters.json", search_params_json.encode("utf-8")
            )

        os.chmod(file_path, 0o644)
        total_sequences = sum(
            content.count(">") for content in fasta_files.values()
        )
        file_size_bytes = file_path.stat().st_size
        del (
            fasta_files
        )  # Release large in-memory data before return so worker exits quickly
        logger.info(
            f"Background FASTA download: Completed. File size: {file_size_bytes} bytes. Sequences: {total_sequences}"
        )
        ...
        logger.error(
            f"Background FASTA download failed: {str(e)}", exc_info=True
        )
        return {
            "success": False,
            "error": str(e),
