...
def sort_vh_families(families):
    ...
    return sorted(
        families,
        key=lambda x: (
            not str(x).startswith("IGHV"),
            extract_family_number(str(x)),
        ),
    )
...
# Load data
vh_marginals, vl_marginals, vh_vl_joint, expected_joint, comparison = (
    load_paired_statistics()
)
...
if vh_marginals is not None:
    ...
    **Mathematical Formulation:**
    ...
    if vh_marginals is not None and vl_marginals is not None:
        ...
        with col1:
            st.markdown(
                "#### V<sub>H</sub> Family Frequencies", unsafe_allow_html=True
            )
            st.dataframe(
                vh_marginals[
                    ["vh_family", "vh_family_count", "vh_family_frequency"]
                ]
                .rename(
                    columns={
                        "vh_family": "Family",
        ...
        with col2:
            st.markdown(
                "#### V<sub>L</sub> Family Frequencies", unsafe_allow_html=True
            )
            st.dataframe(
                vl_marginals[
                    ["vl_family", "vl_family_count", "vl_family_frequency"]
                ]
                .rename(
                    columns={
                        "vl_family": "Family",
    ...
    **Mathematical Formulation:**
    ...
    if vh_vl_joint is not None:
        observed_matrix = vh_vl_joint.pivot(
            index="vh_family", columns="vl_family", values="pair_frequency"
        )
        vh_order = sort_vh_families(observed_matrix.index)
        vl_order = sort_vl_families(observed_matrix.columns)
        observed_matrix = observed_matrix.reindex(
            index=vh_order, columns=vl_order
        )
    ...
    **Interpretation:**
    ...
    if comparison is not None:
        ratio_matrix = comparison.pivot(
            index="vh_family",
            columns="vl_family",
            values="pairing_correction_ratio",
        )
        diff_matrix = comparison.pivot(
            index="vh_family",
            columns="vl_family",
            values="pair_freq_minus_expected",
        )
        ...
        with col1:
            ...
            fig_ratio.update_layout(
                title=dict(
                    text=r"R<sub>i,j</sub> = P<sub>obs</sub>/P<sub>exp</sub>",
                    x=0.5,
                    font=dict(size=16),
                ),
                xaxis=dict(title="V<sub>L</sub> Family", tickangle=45),
        ...
        with col2:
            ...
            fig_diff = go.Figure(
                data=go.Heatmap(
                    ...
                    zmin=-diff_max,
                    zmax=diff_max,
                    colorbar=dict(title="Difference", title_side="right"),
            ...
            fig_diff.update_layout(
                title=dict(
                    text=r"P<sub>obs</sub> - P<sub>exp</sub>",
                    x=0.5,
                    font=dict(size=16),
                ),
                xaxis=dict(title="V<sub>L</sub> Family", tickangle=45),
    ...
    **Mathematical Formulation:**
    ...
    if vh_marginals is not None and heavy_unpaired is not None:
        vh_merged = vh_marginals.merge(
            heavy_unpaired.rename(
                columns={
                    "v_family": "vh_family",
                    "frequency": "unpaired_frequency",
                }
            ),
            on="vh_family",
            how="left",
        ...
        vh_merged_sorted = (
            vh_merged.set_index("vh_family").reindex(vh_order).reset_index()
        )

        fig_vh_comparison.add_trace(
            go.Bar(
        ...
        fig_vh_comparison.update_layout(
            title=dict(
                text="V<sub>H</sub> Family Frequency: Paired vs Unpaired",
                x=0.5,
                font=dict(size=18),
            ),
    ...
    if vl_marginals is not None and light_unpaired is not None:
        ...
        vl_merged_sorted = (
            vl_merged.set_index("vl_family").reindex(vl_order).reset_index()
        )

        fig_vl_comparison.add_trace(
            go.Bar(
        ...
        fig_vl_comparison.update_layout(
            title=dict(
                text="V<sub>L</sub> Family Frequency: Paired vs Unpaired",
                x=0.5,
                font=dict(size=18),
            ),
            xaxis=dict(title="V<sub>L</sub> Family", tickangle=45),
    ...
    **Light-to-Heavy (l_to_h):** For each V<sub>L</sub> family, normalize adjusted frequencies across all V<sub>H</sub> families
    ...
    if adj_freq_df is not None and not adj_freq_df.empty:
        h_to_l_matrix = adj_freq_df.pivot(
            index="vh_family", columns="vl_family", values="h_to_l"
        )
        l_to_h_matrix = adj_freq_df.pivot(
            index="vh_family", columns="vl_family", values="l_to_h"
        )
        ...
        with col1:
            ...
            **P(V<sub>L</sub> | V<sub>H</sub>)**
            ...
            fig_h_to_l.update_layout(
                title=dict(
                    text="P(V<sub>L</sub> | V<sub>H</sub>)",
                    x=0.5,
                    font=dict(size=16),
                ),
                xaxis=dict(title="V<sub>L</sub> Family", tickangle=45),
        ...
        with col2:
            ...
            **P(V<sub>H</sub> | V<sub>L</sub>)**
            ...
            fig_l_to_h.update_layout(
                title=dict(
                    text="P(V<sub>H</sub> | V<sub>L</sub>)",
                    x=0.5,
                    font=dict(size=16),
                ),
                xaxis=dict(title="V<sub>L</sub> Family", tickangle=45),
    ...
    **Final Output:** The `adj_freq_table2.parquet` file contains 144 family pairs with:
    ...
    st.error(
        "Could not load precomputed statistics. Please ensure the files exist in `public/statistics/vh_vl/`."
    )
