...
def should_delete_file(file_path: Path) -> bool:
    ...
    try:
        ...
        if file_age > 86400:  # 24 hours
            logger.warning(
                f"File {file_path.name} is older than 24 hours, deleting as safety measure"
            )
            return True
    except OSError as e:
    ...
    for file_path in download_dir.iterdir():
        ...
        if should_delete_file(file_path):
            try:
                ...
                if dry_run:
                    logger.info(
                        f"[DRY RUN] Would delete: {file_path.name} ({file_size_mb:.2f} MB)"
                    )
                    if marker_path.exists():
                        logger.info(
                            f"[DRY RUN] Would delete marker: {marker_path.name}"
                        )
                else:
                    file_path.unlink()
                    logger.info(
                        f"Deleted: {file_path.name} ({file_size_mb:.2f} MB)"
                    )
                    ...
                    if marker_path.exists():
                        try:
                            ...
                            logger.warning(
                                f"Failed to delete marker {marker_path.name}: {e}"
                            )
...
def main():
    ...
    if args.once:
        ...
        logger.info(
            f"Cleanup complete. {deleted} file(s) {'would be ' if args.dry_run else ''}deleted."
        )
        return

    # Continuous mode
    logger.info(
        f"Starting cleanup daemon for {download_dir} (interval: {args.interval}s)"
    )
    try:
        while True:
            deleted = cleanup_downloads(download_dir, dry_run=args.dry_run)
            if deleted > 0:
                logger.info(
                    f"Cleanup cycle complete. {deleted} file(s) {'would be ' if args.dry_run else ''}deleted."
                )
