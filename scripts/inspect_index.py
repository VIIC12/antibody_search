...
def inspect_index():
    ...
    if "index" in index_data:
        ...
        print(
            f"  Total JSON files: {index_data.get('total_json_files', 'Unknown')}"
        )
        print(
            f"  Total with metadata: {index_data.get('total_with_metadata', 'Unknown')}"
        )
        ...
        for study_info in index.values():
            for metadata in study_info.metadata.values():
                for key, value in metadata.items():
                    ...
                    if (
                        len(field_samples[key]) >= 5
                    ):  # Limit to 5 examples per field
                        continue
