...
def create_unpaired_search_form(selected_databases: list) -> Dict[str, Any]:
    ...
    if chain_type == "Heavy":
        ...
        params, errors, valid = create_light_chain_form(
            prefix="light_", show_title=False
        )
        validation_errors.extend(errors)
...
def search_page_content():
    ...
    if total_databases == 0:
        st.error(
            "No databases found! Please run the data conversion script first."
        )
        st.stop()
    ...
    def render_cached_results_for_mode(
        ...
        if mode == "dual_unpaired":
            ...
            render_dual_unpaired_results(
                heavy_result, light_result, engine_obj, show_toast=False
            )
            return True
    ...
    if not is_ready:
        if "last_search_results" in st.session_state:
            cached_results = st.session_state["last_search_results"]
            if render_cached_results_for_mode(
                cached_results, current_mode=search_mode
            ):
                st.warning(
                    "⚠️ Database is loading. Showing cached results below."
                )
        return
    ...
    with st.form("search_form"):
        if validation_errors:
            st.error(
                f"❌ **Please fix the following errors before searching:** {', '.join(validation_errors)}"
            )
    ...
    if search_mode == "dual_unpaired":
        ...
        if not heavy_info.get("valid", True) or not light_info.get(
            "valid", True
        ):
            st.error(
                "❌ Please fix the invalid inputs (marked in red) before searching"
            )
            return
        ...
        with st.spinner("Searching databases..."):
            try:
                heavy_sequences_df, heavy_stats_df, heavy_statistics = (
                    execute_search_with_stats(
                        engine, heavy_info["search_params"], False, sample_limit
                    )
                )
                light_sequences_df, light_stats_df, light_statistics = (
                    execute_search_with_stats(
        ...
        with results_container.container():
            render_dual_unpaired_results(
                heavy_result, light_result, engine, show_toast=True
            )
        return
    ...
    with st.spinner("Searching database..."):
        try:
            sequences_sample_df, stats_df, statistics = (
                execute_search_with_stats(
