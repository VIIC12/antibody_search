...
def get_nginx_log_path() -> Path:
    ...
    log_path = os.getenv(
        "NGINX_ACCESS_LOG", "/var/log/nginx/downloads_access.log"
    )
...
def process_nginx_logs(
    download_dir: Path, log_path: Path, last_position: int = 0
) -> int:
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Mark downloaded files for cleanup"
    )
    parser.add_argument(
        "--interval",
        type=int,
    ...
    parser.add_argument(
        "--once", action="store_true", help="Process logs once and exit"
    )
    ...
    logger.info(
        f"Starting download marker service (checking {log_path} every {args.interval}s)"
    )
    last_position = 0

    try:
        while True:
            last_position = process_nginx_logs(
                download_dir, log_path, last_position
            )
