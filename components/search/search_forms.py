...
GEN_LKVJ_ERROR_TEXT = (
    "Invalid character(s)! Allowed are: 0123456789 - , * L and K"
)
...
# Motif
MOTIF_PLACEHOLDER = (
    "e.g., *TT or YY.D.*G or YY.{2-6}G or [GYW]TT or .{6}*[FI]W.{2}*"
)
...
def validate_gene_range(
    gene_str: str, gene_type: str
    ...
    if gene_type in ("light_v", "light_j"):
        ...
        if gene_type == "light_v":
            ...
            if invalid:
                return (
                    False,
                    "IGLV 1-11 (Lambda), IGKV 1-7 (Kappa). Invalid: "
                    + "; ".join(invalid),
                )
        else:  # light_j
            for prefix, n in tokens:
            ...
            if invalid:
                return (
                    False,
                    "IGLJ 1-7 (Lambda), IGKJ 1-5 (Kappa). Invalid: "
                    + "; ".join(invalid),
                )
        return True, None
    # Heavy chain
    ...
    if out_of_range:
        return (
            False,
            f"{name} allows {lo}-{hi}. Invalid: {', '.join(str(n) for n in sorted(set(out_of_range)))}",
        )
    return True, None
...
def validate_gene_range_oas(
    ...
    if gene_type == "light_v":
        tokens = _parse_light_gene_tokens(gene_str)
        disallowed_set = (
            LIGHT_V_LAMBDA_OAS_DISALLOWED_PAIRED
            if is_paired
            else LIGHT_V_LAMBDA_OAS_DISALLOWED_UNPAIRED
        )
        invalid = []
        for prefix, n in tokens:
        ...
        if invalid:
            ...
            return (
                False,
                f"{msg} not present in the {dataset_note}. Invalid: {', '.join(invalid)}",
            )
        return True, None
    if gene_type == "light_j":
        ...
        if invalid:
            return (
                False,
                "IGLJ4, IGLJ5 are not present in the OAS dataset. Invalid: "
                + ", ".join(invalid),
            )
        return True, None
    return True, None
...
def validate_gene_input(gene_str: str, field_name: str = "") -> bool:
    ...
    is_light_chain = (
        "light" in field_name.lower() or "igl" in field_name.lower()
    )
...
def validate_cdr_length_input(length_str: str) -> Tuple[bool, Optional[str]]:
    ...
    return (
        False,
        "Invalid format. Use: number (e.g., '2'), range (e.g., '2-5'), or comparison (e.g., '>2', '<5', '>=2', '<=10')",
    )


def parse_cdr_length_condition(
    column_name: str, length_str: str
) -> Optional[str]:
    """
...
def validate_motif_input(motif_str: str) -> bool:
    ...
    pattern = re.compile(
        r"^[ACDEFGHIKLMNPQRSTVWY\.\*\s\{\}\-\[\]0-9]+$", re.IGNORECASE
    )
...
def create_heavy_chain_form(
    prefix: str = "", disabled: bool = False
        ...
        if v and not validate_gene_input(v):
            ...
            if not range_ok and range_err:
                ...
                validation_errors.append(
                    f"{'Heavy ' if prefix else ''}IGHV Gene"
                )
    ...
    with col2:
        ...
        if d and not validate_gene_input(d):
            ...
            if not range_ok and range_err:
                ...
                d_valid = False
                validation_errors.append(
                    f"{'Heavy ' if prefix else ''}IGHD Gene"
                )
    ...
    with col3:
        ...
        if j and not validate_gene_input(j):
            ...
            if not range_ok and range_err:
                ...
                j_valid = False
                validation_errors.append(
                    f"{'Heavy ' if prefix else ''}IGHJ Gene"
                )

    # CDR Length fields
    st.markdown(
        "<h5 style='margin-top: 0.75rem; margin-bottom: 0.25rem;'>CDR Lengths (amino acids)</h5>",
        unsafe_allow_html=True,
    )
    col1, col2, col3, col4 = st.columns(4)

    cdr1_length_key = (
        f"{prefix}cdr1_length_input" if prefix else "cdr1_length_input"
    )
    cdr2_length_key = (
        f"{prefix}cdr2_length_input" if prefix else "cdr2_length_input"
    )
    cdr3_length_key = (
        f"{prefix}cdr3_length_input" if prefix else "cdr3_length_input"
    )
    ...
    with col1:
        ...
        if cdr1_length:
            ...
            if not is_valid:
                ...
                validation_errors.append(
                    f"{'Heavy ' if prefix else ''}CDRH1 Length"
                )
            else:
    ...
    with col2:
        ...
        if cdr2_length:
            ...
            if not is_valid:
                ...
                validation_errors.append(
                    f"{'Heavy ' if prefix else ''}CDRH2 Length"
                )
            else:
    ...
    with col3:
        ...
        if cdr3_length:
            ...
            if not is_valid:
                ...
                validation_errors.append(
                    f"{'Heavy ' if prefix else ''}CDRH3 Length"
                )
            else:
    ...
    # CDR Motif fields
    st.markdown(
        "<h5 style='margin-top: 0.75rem; margin-bottom: 0.25rem;'>CDR Sequence Motifs</h5>",
        unsafe_allow_html=True,
    )
    ...
    # CDR1 Motif
    cdr1_motif_key = (
        f"{prefix}cdr1_motif_input" if prefix else "cdr1_motif_input"
    )
    cdr1_similarity_key = (
        f"{prefix}cdr1_similarity_toggle"
        if prefix
        else "cdr1_similarity_toggle"
    )
    cdr1_mismatches_key = (
        f"{prefix}cdr1_mismatches_input" if prefix else "cdr1_mismatches_input"
    )
    ...
    with col1:
        ...
        with col_toggle:
            if not cdr1_motif and st.session_state.get(
                cdr1_similarity_key, False
            ):
                st.session_state[cdr1_similarity_key] = False

            similarity_help_text = (
                "Enable similarity-based matching (requires motif input)"
                if not cdr1_motif
                else "Enable similarity-based matching. Similar amino acids: A/I/L/V (Aliphatic), D/E (Acidic), F/W/Y (Aromatic), K/R (Basic), N/Q (Amide), S/T (Hydroxyl). Unique: C, G, H, M, P."
            )
    ...
    # CDR2 Motif
    cdr2_motif_key = (
        f"{prefix}cdr2_motif_input" if prefix else "cdr2_motif_input"
    )
    cdr2_similarity_key = (
        f"{prefix}cdr2_similarity_toggle"
        if prefix
        else "cdr2_similarity_toggle"
    )
    cdr2_mismatches_key = (
        f"{prefix}cdr2_mismatches_input" if prefix else "cdr2_mismatches_input"
    )
    ...
    with col2:
        ...
        col_toggle, col_mismatch = st.columns([2, 1])
        with col_toggle:
            if not cdr2_motif and st.session_state.get(
                cdr2_similarity_key, False
            ):
    ...
    # CDR3 Motif
    cdr3_motif_key = (
        f"{prefix}cdr3_motif_input" if prefix else "cdr3_motif_input"
    )
    cdr3_similarity_key = (
        f"{prefix}cdr3_similarity_toggle"
        if prefix
        else "cdr3_similarity_toggle"
    )
    cdr3_mismatches_key = (
        f"{prefix}cdr3_mismatches_input" if prefix else "cdr3_mismatches_input"
    )
    ...
    with col3:
        ...
        col_toggle, col_mismatch = st.columns([2, 1])
        with col_toggle:
            if not cdr3_motif and st.session_state.get(
                cdr3_similarity_key, False
            ):
    ...
    params = {
        ...
        f"{prefix}cdr1_similarity"
        if prefix
        else "cdr1_similarity": cdr1_similarity,
        f"{prefix}cdr2_similarity"
        if prefix
        else "cdr2_similarity": cdr2_similarity,
        f"{prefix}cdr3_similarity"
        if prefix
        else "cdr3_similarity": cdr3_similarity,
        f"{prefix}cdr1_mismatches"
        if prefix
        else "cdr1_mismatches": cdr1_mismatches,
        f"{prefix}cdr2_mismatches"
        if prefix
        else "cdr2_mismatches": cdr2_mismatches,
        f"{prefix}cdr3_mismatches"
        if prefix
        else "cdr3_mismatches": cdr3_mismatches,
    }
...
def create_light_chain_form(
    prefix: str = "light_", disabled: bool = False
) -> Tuple[Dict[str, Any], List[str], bool]:
    """
    ...
    with col1:
        ...
        col_toggle, col_mismatch = st.columns([2, 1])
        with col_toggle:
            if not light_cdr1_motif and st.session_state.get(
        ...
        with col_mismatch:
            if light_cdr1_motif and light_cdr1_similarity:
                max_mismatches = len(
                    light_cdr1_motif.replace(".", "").replace("*", "")
                )
    ...
    with col2:
        ...
        col_toggle, col_mismatch = st.columns([2, 1])
        with col_toggle:
            if not light_cdr2_motif and st.session_state.get(
        ...
        with col_mismatch:
            if light_cdr2_motif and light_cdr2_similarity:
                max_mismatches = len(
                    light_cdr2_motif.replace(".", "").replace("*", "")
                )
    ...
    with col3:
        ...
        col_toggle, col_mismatch = st.columns([2, 1])
        with col_toggle:
            if not light_cdr3_motif and st.session_state.get(
        ...
        with col_mismatch:
            if light_cdr3_motif and light_cdr3_similarity:
                max_mismatches = len(
                    light_cdr3_motif.replace(".", "").replace("*", "")
                )
    ...
    valid = all([
        light_v_valid,
        light_j_valid,
        light_cdr1_length_valid,
        light_cdr2_length_valid,
        light_cdr3_length_valid,
        light_cdr1_motif_valid,
        light_cdr2_motif_valid,
        light_cdr3_motif_valid,
    ])
