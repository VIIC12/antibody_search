...
try:
    DUCKDB_THREADS = (
        int(_duckdb_threads_env) if _duckdb_threads_env is not None else 4
    )
...
class AntibodySearchEngine:
    ...
    def __init__(
        ...
        self._inferred_lookup: Dict[
            str, Dict[str, Dict[str, List[Tuple[str, float]]]]
        ] = {"V": {"Heavy": {}, "Light": {}}, "J": {"Heavy": {}, "Light": {}}}
        ...
        try:
            ...
            temp_dir = self.conn.execute(
                "SELECT current_setting('temp_directory')"
            ).fetchone()
            _log_debug_event(
                "AntibodySearchEngine.__init__",
                "DuckDB initialized",
                {
                    "db_path": db_path,
                    "temp_directory": str(temp_dir[0])
                    if temp_dir
                    else "unknown",
                    "cwd": os.getcwd(),
                },
        ...
        if verbose:
            print(
                f"DuckDB: memory_limit={DUCKDB_MEMORY_LIMIT}, threads={DUCKDB_THREADS}"
            )
        ...
        # #region agent log
        _log_debug_event(
            "AntibodySearchEngine.__init__", "Calling _register_data", {}
        )
        # #endregion
    ...
    def _register_data(self, progress_callback=None, verbose: bool = True):
        ...
        chain_file_map: Dict[str, List[str]] = {
            "Heavy": [],
            "Light": [],
            "Paired": [],
        }
        ...
        if not parquet_files:
            raise FileNotFoundError(
                f"No Parquet files found in {[str(d) for d in self.data_dirs]}"
            )
        ...
        # #region agent log
        _log_debug_event(
            "AntibodySearchEngine._register_data",
            "Parquet files found",
        ...
        _log_debug_event(
            "AntibodySearchEngine._register_data",
            "Creating antibodies_base view",
            {"files_count": len(parquet_file_paths)},
        )
        ...
        _log_debug_event(
            "AntibodySearchEngine._register_data",
            "antibodies_base view created",
            {"duration": time.time() - start_view},
        )
        # #endregion
        ...
        file_path_expr = (
            "file_path"
            if "file_path" in metadata_columns
            else (
                "filename"
                if "filename" in metadata_columns
                else "NULL AS file_path"
            )
        )
        if "file_path" not in metadata_columns:
            file_path_expr = (
                file_path_expr
                if " AS " in file_path_expr
                else f"{file_path_expr} AS file_path"
            )
        ...
        _log_debug_event(
            "AntibodySearchEngine._register_data",
            "Counting total sequences (metadata)",
            {},
        )
        ...
        _log_debug_event(
            "AntibodySearchEngine._register_data",
            "Total sequences counted",
            {"duration": time.time() - start_count},
        )
        # #endregion
        ...
        # #region agent log
        _log_debug_event(
            "AntibodySearchEngine._register_data", "Chain views created", {}
        )
        # #endregion
    ...
    def load_inferred_pairs(
        self, overlay_directories: Optional[List[Union[str, Path]]]
    ) -> None:
        """
        ...
        for overlay_dir in overlay_directories:
            ...
            v_freq_path = (
                overlay_path
                / "adj_vh_vl_freq_table_for_search_wo_epsilon.parquet"
            )
            if v_freq_path.exists():
                try:
                    ...
                    print(
                        f"⚠️ Failed to load VH-VL inferred pairings from {v_freq_path}: {exc}"
                    )

            # Try to load JH-JL file
            j_freq_path = (
                overlay_path
                / "adj_jh_vj_freq_table_for_search_wo_epsilon.parquet"
            )
            if j_freq_path.exists():
                try:
                    ...
                    print(
                        f"⚠️ Failed to load JH-JL inferred pairings from {j_freq_path}: {exc}"
                    )
        ...
        if v_tables:
            ...
            v_combined = v_combined.rename(
                columns={
                    "vh_family": "heavy_v_family",
                    "vl_family": "light_v_family",
                }
            )
            ...
            if missing_cols:
                print(
                    f"⚠️ VH-VL inferred pairing table missing columns: {missing_cols}"
                )
            else:
                v_combined = v_combined.dropna(
                    subset=["heavy_v_family", "light_v_family"]
                )
                if not v_combined.empty:
                    v_combined["heavy_v_family"] = (
                        v_combined["heavy_v_family"]
                        .astype(str)
                        .str.upper()
                        .str.strip()
                    )
                    v_combined["light_v_family"] = (
                        v_combined["light_v_family"]
                        .astype(str)
                        .str.upper()
                        .str.strip()
                    ...
                    if "h_to_l" in v_combined.columns:
                        v_combined["h_to_l"] = pd.to_numeric(
                            v_combined["h_to_l"], errors="coerce"
                        )
                    if "l_to_h" in v_combined.columns:
                        v_combined["l_to_h"] = pd.to_numeric(
                            v_combined["l_to_h"], errors="coerce"
                        )

                    drop_subset = [
                        col
                        for col in ("h_to_l", "l_to_h")
                        if col in v_combined.columns
                    ]
                    if drop_subset:
                        v_combined = v_combined.dropna(
                            subset=drop_subset, how="all"
                        )
                    ...
                    if not v_combined.empty:
                        ...
                        if agg_dict:
                            v_aggregated = v_combined.groupby(
                                ["heavy_v_family", "light_v_family"],
                                as_index=False,
                            ).agg(agg_dict)
                            ...
                            if "l_to_h" not in v_aggregated.columns:
                                v_aggregated["l_to_h"] = v_aggregated.groupby(
                                    "light_v_family"
                                )["h_to_l"].transform(
                                    lambda values: (
                                        values / values.sum() * 100.0
                                    )
                                    if values.sum()
                                    else values * 0.0
                                )

                            v_aggregated["h_to_l"] = v_aggregated[
                                "h_to_l"
                            ].fillna(0.0)
                            v_aggregated["l_to_h"] = v_aggregated[
                                "l_to_h"
                            ].fillna(0.0)
        ...
        if j_tables:
            ...
            # Rename to internal format
            j_combined = j_combined.rename(
                columns={
                    "jh_family": "heavy_j_family",
                    "jl_family": "light_j_family",
                }
            )
            ...
            if missing_cols:
                print(
                    f"⚠️ JH-JL inferred pairing table missing columns: {missing_cols}"
                )
            else:
                j_combined = j_combined.dropna(
                    subset=["heavy_j_family", "light_j_family"]
                )
                if not j_combined.empty:
                    j_combined["heavy_j_family"] = (
                        j_combined["heavy_j_family"]
                        .astype(str)
                        .str.upper()
                        .str.strip()
                    )
                    j_combined["light_j_family"] = (
                        j_combined["light_j_family"]
                        .astype(str)
                        .str.upper()
                        .str.strip()
                    ...
                    if "h_to_l" in j_combined.columns:
                        j_combined["h_to_l"] = pd.to_numeric(
                            j_combined["h_to_l"], errors="coerce"
                        )
                    if "l_to_h" in j_combined.columns:
                        j_combined["l_to_h"] = pd.to_numeric(
                            j_combined["l_to_h"], errors="coerce"
                        )

                    drop_subset = [
                        col
                        for col in ("h_to_l", "l_to_h")
                        if col in j_combined.columns
                    ]
                    if drop_subset:
                        j_combined = j_combined.dropna(
                            subset=drop_subset, how="all"
                    ...
                    if not j_combined.empty:
                        ...
                        if agg_dict:
                            j_aggregated = j_combined.groupby(
                                ["heavy_j_family", "light_j_family"],
                                as_index=False,
                            ).agg(agg_dict)
                            ...
                            if "l_to_h" not in j_aggregated.columns:
                                j_aggregated["l_to_h"] = j_aggregated.groupby(
                                    "light_j_family"
                                )["h_to_l"].transform(
                                    lambda values: (
                                        values / values.sum() * 100.0
                            ...
                            j_aggregated["h_to_l"] = j_aggregated[
                                "h_to_l"
                            ].fillna(0.0)
                            j_aggregated["l_to_h"] = j_aggregated[
                                "l_to_h"
                            ].fillna(0.0)
    ...
    def _rebuild_inferred_lookup(self) -> None:
        ...
        # Process VH-VL pairings
        if (
            self.inferred_v_pairs_df is not None
            and not self.inferred_v_pairs_df.empty
        ):
            ...
            for heavy_family, group in df.groupby("heavy_v_family"):
                ...
                v_lookup_heavy[heavy_family] = [
                    ...
                    for partner, value in zip(
                        ordered["light_v_family"], ordered["h_to_l"]
                    )
                    if float(value) > 0
                ]
            ...
            for light_family, group in df.groupby("light_v_family"):
                ...
                v_lookup_light[light_family] = [
                    (partner, float(value))
                    for partner, value in zip(
                        ordered["heavy_v_family"], ordered["l_to_h"]
                    )
                    if float(value) > 0
                ]
        ...
        # Process JH-JL pairings
        if (
            self.inferred_j_pairs_df is not None
            and not self.inferred_j_pairs_df.empty
        ):
            ...
            for heavy_family, group in df.groupby("heavy_j_family"):
                ...
                j_lookup_heavy[heavy_family] = [
                    (partner, float(value))
                    for partner, value in zip(
                        ordered["light_j_family"], ordered["h_to_l"]
                    )
                    if float(value) > 0
                ]
            ...
            for light_family, group in df.groupby("light_j_family"):
                ...
                j_lookup_light[light_family] = [
                    (partner, float(value))
                    for partner, value in zip(
                        ordered["heavy_j_family"], ordered["l_to_h"]
                    )
                    if float(value) > 0
                ]
    ...
    def get_inferred_distribution(
        ...
        return self._get_inferred_partners(
            chain_type, gene_or_family, gene_type=gene_type, top_n=top_n
        )

    def _attach_inferred_partners(
        self, df: pd.DataFrame, chain_type: str, top_n: int = 3
    ) -> pd.DataFrame:
        ...
        partner_prefix = (
            "inferred_light"
            if chain_type.lower() == "heavy"
            else "inferred_heavy"
        )

        # Attach V gene inferred partners
        v_lookup = self._inferred_lookup.get("V", {}).get(
        ...
        if v_lookup and "v_call" in df.columns:
            ...
            for gene in df["v_call"]:
                partners = self._get_inferred_partners(
                    chain_type, gene, gene_type="V", top_n=top_n
                )
                if partners:
                    v_summaries.append(
                        ", ".join(
                            f"{partner} ({percent:.1f}%)"
                            for partner, percent in partners
                        )
                    )
        ...
        j_lookup = self._inferred_lookup.get("J", {}).get(
            chain_type.capitalize(), {}
        )
        if j_lookup and "j_call" in df.columns:
            ...
            for gene in df["j_call"]:
                partners = self._get_inferred_partners(
                    chain_type, gene, gene_type="J", top_n=top_n
                )
                if partners:
                    j_summaries.append(
                        ", ".join(
    ...
    def _build_inferred_hint(
        ...
        if any(sep in gene_input for sep in (",", "|")):
            return None
        partners = self._get_inferred_partners(
            chain_type, gene_input, gene_type=gene_type, top_n=top_n
        )
        if not partners:
            return None
        ...
        summary = ", ".join(
            f"{partner} ({percent:.1f}%)" for partner, percent in partners
        )
        return f"{partner_label} {gene_label} families: {summary}"

    def generate_similarity_pattern(
        self, motif: str, max_mismatches: int = 2
    ) -> str:
        """
    ...
    def _parse_cdr_length_condition(
        self, column_name: str, length_str: Optional[str]
    ) -> Optional[str]:
        """
    ...
    def _build_gene_pattern(
        self, column: str, gene: str, force_light_chain: bool = False
    ) -> str:
        """
        ...
        light_chain_type = (
            None  # None = both, 'L' = Lambda only, 'K' = Kappa only
        )
        if is_light_chain:
        ...
        def build_light_chain_pattern(
            gene_pattern: str, is_v: bool, is_j: bool
        ) -> tuple:
        ...
        if gene.endswith("-"):
            ...
            if is_v_gene:
                if is_light_chain:
                    pattern_result = build_light_chain_pattern(
                        gene, True, False
                    )
                else:
                    pattern = f"IGHV{gene}%"
                ...
                pattern = (
                    f"IGHD{gene}%" if not is_light_chain else f"IGKD{gene}%"
                )
            elif is_j_gene:
                if is_light_chain:
                    pattern_result = build_light_chain_pattern(
                ...
                pattern = (
                    f"IGHV{gene}%" if not is_light_chain else f"IGKV{gene}%"
                )
        elif "*" in gene:
    ...
    def search(
        ...
        # #region agent log
        try:
            mem_limit = self.conn.execute(
                "SELECT current_setting('memory_limit')"
            ).fetchone()
            _log_debug_event(
                "AntibodySearchEngine.search",
                "Search started",
                {
                    "chain_mode": chain_mode,
                    "full_results": full_results,
                    "limit": limit,
                    "memory_limit": str(mem_limit[0])
                    if mem_limit
                    else "unknown",
                },
            )
        ...
        logger.debug(
            f"Query started - Chain mode: {chain_mode}, Full results: {full_results}, Limit: {limit}"
        )
        # Add parameter logging
        logger.debug(
            f"Search params - Heavy V: {heavy_v}, J: {heavy_j}, Light V: {light_v}, J: {light_j}"
        )
        ...
        if resolved_mode not in {"paired", "heavy", "light"}:
            resolved_mode = (
                "paired"
                if self.schema.get("search_type") == "paired"
                else "heavy"
            )
        if (
            resolved_mode == "paired"
            and self.schema.get("search_type") != "paired"
        ):
            resolved_mode = "heavy"
        ...
        if resolved_mode == "paired":
            return self._search_paired(
                ...
                heavy_cdr2_motif,
                heavy_cdr3_motif,
                heavy_cdr1_similarity,
                heavy_cdr2_similarity,
                heavy_cdr3_similarity,
                heavy_cdr1_mismatches,
                heavy_cdr2_mismatches,
                heavy_cdr3_mismatches,
                light_v,
                light_d,
                light_j,
                light_cdr1_length,
                ...
                light_cdr2_motif,
                light_cdr3_motif,
                light_cdr1_similarity,
                light_cdr2_similarity,
                light_cdr3_similarity,
                light_cdr1_mismatches,
                light_cdr2_mismatches,
                light_cdr3_mismatches,
                full_results,
                limit,
                columns,
                start_time,
            ...
            cdr1_len = (
                light_cdr1_length
                if chain_type == "Light"
                else heavy_cdr1_length
            )
            cdr2_len = (
                light_cdr2_length
                if chain_type == "Light"
                else heavy_cdr2_length
            )
            cdr3_len = (
                light_cdr3_length
                if chain_type == "Light"
                else heavy_cdr3_length
            )
            cdr1_motif_val = (
                light_cdr1_motif if chain_type == "Light" else heavy_cdr1_motif
            )
            cdr2_motif_val = (
                light_cdr2_motif if chain_type == "Light" else heavy_cdr2_motif
            )
            cdr3_motif_val = (
                light_cdr3_motif if chain_type == "Light" else heavy_cdr3_motif
            )
            cdr1_sim = (
                light_cdr1_similarity
                if chain_type == "Light"
                else heavy_cdr1_similarity
            )
            cdr2_sim = (
                light_cdr2_similarity
                if chain_type == "Light"
                else heavy_cdr2_similarity
            )
            cdr3_sim = (
                light_cdr3_similarity
                if chain_type == "Light"
                else heavy_cdr3_similarity
            )
            cdr1_mm = (
                light_cdr1_mismatches
                if chain_type == "Light"
                else heavy_cdr1_mismatches
            )
            cdr2_mm = (
                light_cdr2_mismatches
                if chain_type == "Light"
                else heavy_cdr2_mismatches
            )
            cdr3_mm = (
                light_cdr3_mismatches
                if chain_type == "Light"
                else heavy_cdr3_mismatches
            )
    ...
    def _search_unpaired(
        ...
        conditions = []
        is_light_unpaired = (
            chain_type.lower() == "light" if chain_type else False
        )
        ...
        if (
            chain_type
            and "chain" in self.schema.get("available_columns", [])
            and not is_chain_specific_view
        ):
            conditions.append(f"chain = '{chain_type}'")

        # Narrow scan to the selected directories for the requested chain when possible
        source_prefixes = (
            self.chain_dir_prefixes.get(chain_type, set())
            if chain_type
            else set()
        )
        if source_prefixes and "source_file" in self.schema.get(
            "available_columns", []
        ):
            source_conditions = []
            for prefix in source_prefixes:
                ...
                source_conditions.append(
                    f"source_file LIKE '{escaped_prefix}%'"
                )
            if source_conditions:
                conditions.append(f"({' OR '.join(source_conditions)})")

        def _collect_gene_conditions(
            column: str, value: str, force_light: bool
        ) -> Optional[str]:
            if not value:
                return None
            parts = [
            ...
            if not parts:
                return None
            patterns = [
                self._build_gene_pattern(
                    column, part, force_light_chain=force_light
                )
                for part in parts
            ]
        ...
        if cdr1_length is not None:
            condition = self._parse_cdr_length_condition(
        ...
        if cdr1_motif:
            if cdr1_similarity:
                ...
                regex_pattern = self.generate_similarity_pattern(
                    cdr1_motif, cdr1_mismatches
                )
            else:
        ...
        logger.debug(
            f"Unpaired WHERE clause constructed. Length: {len(where_clause)}"
        )
        ...
        if full_results:
            ...
            if columns:
                logger.debug(
                    f"Plotting/restricted columns for unpaired: {columns}"
                )
            select_list = (
            ...
            # #region agent log
            _log_debug_event(
                "AntibodySearchEngine._search_unpaired",
            ...
            if not columns:
                results_df = self._attach_inferred_partners(
                    results_df, chain_type
                )
            ...
            total_hits = (
                int(stats_df["hits"].sum()) if not stats_df.empty else 0
            )
        ...
        try:
            ...
            if chain_type:
                chain_filter_clause = (
                    f" WHERE lower(chain) = '{chain_type.lower()}'"
                )
            metadata_query = f"""
        ...
        if (
            total_sequences_lookup is not None
            and not total_sequences_lookup.empty
        ):
            ...
            if "total" in total_sequences_lookup.columns:
                all_subjects_df = total_sequences_lookup.rename(
                    columns={"total": "total_sequences"}
                ).copy()
            else:
            ...
            if not stats_df.empty:
                ...
                if "hits" not in stats_df.columns:
                    stats_df["hits"] = 0
                if (
                    "num_files" not in stats_df.columns
                    and "num_files" in all_subjects_df.columns
                ):
                ...
                if "num_files" in stats_df.columns:
                    stats_df["num_files"] = (
                        stats_df["num_files"].fillna(0).astype(int)
                    )
            else:
            ...
            stats_df["total_sequences"] = (
                stats_df["total_sequences"].fillna(0).astype(int)
            )
        ...
        statistics = {
            ...
            "percentage": round(total_hits / self.total_sequences * 100, 2)
            if self.total_sequences > 0
            else 0,
            "per_million": round(total_hits / self.total_sequences * 1000000, 1)
            if self.total_sequences > 0
            else 0,
        ...
        if overlay_active:
            ...
            if ighv:
                v_hint = self._build_inferred_hint(
                    ighv, chain_type, gene_type="V"
                )
                if v_hint:
                    hints.append(v_hint)
            # Build J gene hint if J gene is searched
            if ighj:
                j_hint = self._build_inferred_hint(
                    ighj, chain_type, gene_type="J"
                )
                if j_hint:
                    hints.append(j_hint)
        ...
        if verbose:
            self._print_search_summary(
                statistics, chain_type, full_results, limit
            )
        ...
        try:
            ...
            results_size = (
                results_df.memory_usage(deep=True).sum()
                if not results_df.empty
                else 0
            )
            stats_size = (
                stats_df.memory_usage(deep=True).sum()
                if not stats_df.empty
                else 0
            )
    ...
    def _search_paired(
        ...
        if heavy_v:
            ...
            if separator in heavy_v:
                    ...
                    for col in self.schema["chain_columns"]["v_call"]:
                        if "_heavy" in col:
                            heavy_v_conditions.append(
                                self._build_gene_pattern(col, g)
                            )
                ...
                for col in self.schema["chain_columns"]["v_call"]:
                    if "_heavy" in col:
                        heavy_v_conditions.append(
                            self._build_gene_pattern(col, heavy_v)
                        )
                if heavy_v_conditions:
                    conditions.append(f"({' OR '.join(heavy_v_conditions)})")
        ...
        if heavy_d:
            ...
            if separator in heavy_d:
                ...
                for col in self.schema["chain_columns"]["d_call"]:
                    if "_heavy" in col:
                        heavy_d_conditions.append(
                            self._build_gene_pattern(col, heavy_d)
                        )
                if heavy_d_conditions:
                    conditions.append(f"({' OR '.join(heavy_d_conditions)})")
        ...
        if heavy_cdr1_motif:
            if heavy_cdr1_similarity:
                # Use similarity pattern with mismatch tolerance
                regex_pattern = self.generate_similarity_pattern(
                    heavy_cdr1_motif, heavy_cdr1_mismatches
                )
            else:
                # Exact pattern matching - mismatches parameter is not applicable
        ...
        if heavy_cdr2_motif:
            if heavy_cdr2_similarity:
                # Use similarity pattern with mismatch tolerance
                regex_pattern = self.generate_similarity_pattern(
                    heavy_cdr2_motif, heavy_cdr2_mismatches
                )
            else:
                # Exact pattern matching - mismatches parameter is not applicable
        ...
        if heavy_cdr3_motif:
            if heavy_cdr3_similarity:
                # Use similarity pattern with mismatch tolerance
                regex_pattern = self.generate_similarity_pattern(
                    heavy_cdr3_motif, heavy_cdr3_mismatches
                )
            else:
                # Exact pattern matching - mismatches parameter is not applicable
            ...
            if separator in light_v:
                ...
                for col in self.schema["chain_columns"]["v_call"]:
                    if "_light" in col:
                        light_v_conditions.append(
                            self._build_gene_pattern(
                                col, light_v, force_light_chain=True
                            )
                        )
                if light_v_conditions:
                    conditions.append(f"({' OR '.join(light_v_conditions)})")
            ...
            if separator in light_d:
                ...
                for col in self.schema["chain_columns"]["d_call"]:
                    if "_light" in col:
                        light_d_conditions.append(
                            self._build_gene_pattern(
                                col, light_d, force_light_chain=True
                            )
                        )
                if light_d_conditions:
                    conditions.append(f"({' OR '.join(light_d_conditions)})")
            ...
            if separator in light_j:
                ...
                for col in self.schema["chain_columns"]["j_call"]:
                    if "_light" in col:
                        light_j_conditions.append(
                            self._build_gene_pattern(
                                col, light_j, force_light_chain=True
                            )
                        )
                if light_j_conditions:
                    conditions.append(f"({' OR '.join(light_j_conditions)})")
        ...
        if light_cdr1_motif:
            if light_cdr1_similarity:
                # Use similarity pattern with mismatch tolerance
                regex_pattern = self.generate_similarity_pattern(
                    light_cdr1_motif, light_cdr1_mismatches
                )
            else:
                # Exact pattern matching - mismatches parameter is not applicable
        ...
        if light_cdr2_motif:
            if light_cdr2_similarity:
                # Use similarity pattern with mismatch tolerance
                regex_pattern = self.generate_similarity_pattern(
                    light_cdr2_motif, light_cdr2_mismatches
                )
            else:
                # Exact pattern matching - mismatches parameter is not applicable
        ...
        if light_cdr3_motif:
            if light_cdr3_similarity:
                # Use similarity pattern with mismatch tolerance
                regex_pattern = self.generate_similarity_pattern(
                    light_cdr3_motif, light_cdr3_mismatches
                )
            else:
                # Exact pattern matching - mismatches parameter is not applicable
        ...
        # Log WHERE clause length
        logger.debug(
            f"Paired WHERE clause constructed. Length: {len(where_clause)}"
        )
        ...
        if full_results:
            ...
            if columns:
                logger.debug(
                    f"Plotting/restricted columns for paired: {columns}"
                )
            select_list = (
                ", ".join(f'"{c}"' for c in columns) if columns else "*"
            )
            ...
            # #region agent log
            _log_debug_event(
                "AntibodySearchEngine._search_paired",
                "Sample query completed",
                {
                    "duration": time.time() - start_sample,
                    "rows": len(results_df) if not results_df.empty else 0,
        ...
        if verbose:
            self._print_search_summary(
                statistics, "paired", full_results, limit
            )
    ...
    def _print_search_summary(
        ...
        print(
            f"{mode_str} search completed: {hits_str} hits found ({percentage:.2f}%, {per_million:.0f} per million)"
        )
