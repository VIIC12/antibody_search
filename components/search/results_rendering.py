...
def render_search_results(
    ...
    render_subject_statistics(
        stats_df, statistics, search_params=search_params, is_paired=is_paired
    )
    render_results_plots(
        sequences_sample_df, statistics, is_paired, search_params, engine
    )
...
def render_subject_statistics(
    ...
    if stats_df is not None and not stats_df.empty:
        ...
        if {"total_sequences", "hits"}.issubset(working_df.columns):
            ...
            if (
                not plot_meta.get("error")
                and plot_filtered_df is not None
                and not plot_filtered_df.empty
            ):
    ...
    with content_col:
        if stats_df is not None and not stats_df.empty:
            ...
            ordered_columns = [
                col for col in desired_order if col in stats_df.columns
            ]
            stats_df_display = (
                stats_df[ordered_columns] if ordered_columns else stats_df
            )
    ...
    with summary_col:
        if not plot_summary_df.empty:
            ...
            st.info(
                "Plot summary unavailable (no donors pass the sequence threshold)."
            )

        toggle_col1, toggle_col2 = st.columns(2)
        # Lock while search/plots are busy — toggling mid-run can crash the Streamlit server
        donor_plot_toggles_locked = st.session_state.get(
            "search_status"
        ) == "running" or bool(
            st.session_state.get("plotting_controls_locked", False)
        )
        with toggle_col1:
            st.toggle(
...
def render_sequences_table(
    ...
    if not sequences_sample_df.empty:
        ...
        sequences_display = format_results_dataframe(
            sequences_sample_df, is_paired
        )
        ...
        if total_hits == 0:
            st.info(
                ":material/search_off: **No sequences found matching your search criteria.** Try adjusting your search parameters."
            )
        else:
...
def render_stats_download_button(
    ...
    widget_key = (
        key
        or f"stats_download_{(statistics or {}).get('total_hits', 0)}_{hash(str(search_params))}"
    )
...
def render_full_download_button(
    ...
    if not sequences_sample_df.empty:
        ...
        loadable_databases = (
            ...
            or session_selected_dbs
        )

        for db_path in session_selected_dbs:
            try:
                path_obj = Path(db_path)
                selected_databases_formatted.append(
        ...
        search_params_with_metadata["selected_databases"] = (
            selected_databases_formatted
        )
        ...
        if is_paired:
            ...
            if any("Light" in db for db in selected_databases_formatted):
                ...
                light_keys = [
                    key
                    for key in base_search_params.keys()
        ...
        if full_status_key not in st.session_state:
            st.session_state[full_status_key] = (
        ...
        with col_full:
            if status == "idle":
                if st.button(
                    button_label, width="stretch", key=f"{download_key}_button"
                ):
                    ...
                    if result.get("success"):
                        st.toast(
                            "Full results table is ready for download!",
                            icon="⬇",
                        )
                    st.rerun()
                ...
                if st.button(
                    "🔄 Retry", key=f"{download_key}_retry", width="stretch"
                ):
        ...
        with col_new:
            if new_status == "idle":
                if st.button(
                    ...
                    st.session_state[new_status_key] = (
                        "completed" if result.get("success") else "failed"
                    )
                    if result.get("success"):
                ...
                if result and result.get("success"):
                    ...
                    st.button(
                        "❌ Generation Failed",
                        disabled=True,
                        key=f"{new_download_key}_failed",
                        width="stretch",
                    )
                    error_msg = (
                        result.get("error", "Unknown error")
                ...
                if st.button(
                    "🔄 Retry", key=f"{new_download_key}_retry", width="stretch"
                ):
...
def _render_selected_databases_summary(
    selected_databases: Optional[List[Any]],
) -> None:
...
def render_search_criteria_display(
    ...
    if is_paired:
        ...
        with col1:
            ...
            _render_chain_criteria(
                formatted_params, "heavy_", chain_type="Heavy"
            )

        with col2:
            icon_heading("light", "Light Chain", 4, margin_top=0.5)
            _render_chain_criteria(
                formatted_params, "light_", chain_type="Light"
            )
        return

    chain_type = _determine_unpaired_chain_type(
        search_params, selected_databases
    )
    formatted_params = _format_query_params_for_display(search_params, False)
    prefix = "heavy_" if chain_type == "Heavy" else "light_"
    icon_heading(
        f"{chain_type.lower()}", f"{chain_type} Chain", 4, margin_top=0.5
    )
    _render_chain_criteria(formatted_params, prefix, chain_type=chain_type)


def _render_chain_criteria(
    search_params: Dict[str, Any], prefix: str, chain_type: str = "Heavy"
) -> None:
    """
    ...
    for cdr in [1, 2, 3]:
        ...
        if motif:
            similarity = search_params.get(
                f"{prefix}cdr{cdr}_similarity", False
            )
            mismatches = search_params.get(f"{prefix}cdr{cdr}_mismatches", 0)
            motif_str = f"**CDR{cdr} Motif:** {motif}"
            if similarity:
                motif_str += (
                    f" (Similarity search, max {mismatches} mismatches)"
                )
...
def _render_chain_criteria_compact(
    search_params: Dict[str, Any], prefix: str, chain_type: str = "Heavy"
) -> None:
    """
...
def render_search_parameters_expander(
    statistics: Dict[str, Any], is_paired: bool
) -> None:
    """
    ...
    with st.expander(":material/search: Search Parameters"):
        ...
        formatted_params = _format_query_params_for_display(
            statistics.get("query_params", {}), is_paired
        )
        st.json(formatted_params)


def _determine_unpaired_chain_type(
    search_params: Dict[str, Any], selected_databases: List[str]
) -> str:
    ...
    chain_type = (
        ...
        or (
            "Light"
            if any("/Light/" in str(db) for db in selected_databases)
            else "Heavy"
        )
    )
    ...
    if search_params.get("light_v") or search_params.get("light_j"):
        return "Light"
    if selected_databases and any(
        "/Light/" in str(db) for db in selected_databases
    ):
        return "Light"
    return "Heavy"


def _format_query_params_for_display(
    query_params: Dict[str, Any], is_paired: bool
) -> Dict[str, Any]:
    ...
    chain_type = (
        formatted.get("chain_type")
        or formatted.get("unpaired_chain_type")
        or "Heavy"
    )
    chain_type = chain_type.capitalize()
...
def render_dual_search_criteria_display(
    ...
    heavy_params = _format_query_params_for_display(
        heavy_statistics.get("query_params", {}), False
    )
    light_params = _format_query_params_for_display(
        light_statistics.get("query_params", {}), False
    )
...
def render_dual_search_parameters_expander(
    heavy_statistics: Dict[str, Any], light_statistics: Dict[str, Any]
) -> None:
    ...
    with st.expander(":material/search: Search Parameters"):
        ...
        heavy_formatted = _format_query_params_for_display(
            heavy_statistics.get("query_params", {}), False
        )
        heavy_formatted.pop("chain_type", None)
        heavy_filtered = {
            key: value
            for key, value in heavy_formatted.items()
        ...
        light_formatted = _format_query_params_for_display(
            light_query_params, False
        )
        light_formatted.pop("chain_type", None)
        light_filtered = {
            key: value
            for key, value in light_formatted.items()
            if key.startswith("light_")
        }
        if not light_filtered:
            ...
            for base_key, prefix in (
                ("ighv", "light_v"),
                ("ighj", "light_j"),
                ("cdr1_length", "light_cdr1_length"),
                ("cdr2_length", "light_cdr2_length"),
                ("cdr3_length", "light_cdr3_length"),
                ("cdr1_motif", "light_cdr1_motif"),
                ("cdr2_motif", "light_cdr2_motif"),
                ("cdr3_motif", "light_cdr3_motif"),
                ("cdr1_similarity", "light_cdr1_similarity"),
                ("cdr2_similarity", "light_cdr2_similarity"),
                ("cdr3_similarity", "light_cdr3_similarity"),
                ("cdr1_mismatches", "light_cdr1_mismatches"),
                ("cdr2_mismatches", "light_cdr2_mismatches"),
...
def render_dual_unpaired_results(
    ...
    render_dual_search_parameters_expander(
        heavy_result["statistics"], light_result["statistics"]
    )
