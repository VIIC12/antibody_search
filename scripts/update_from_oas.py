...
def get_json_files_from_study(base_url: str, study_path: str) -> List[FileInfo]:
    ...
    for json_subdir in json_subdirs:
        ...
        try:
            ...
            if json_files:  # Found JSON files in this subdirectory
                logger.debug(
                    f"Found {len(json_files)} JSON files in {json_url}"
                )
                return json_files
            else:
    ...
    logger.warning(
        f"Failed to get JSON files from {study_path} - tried both json/ and json_paired/"
    )
    return []
...
def update_json_index_incrementally(
    ...
    for directory, is_paired in directories_to_scan:
        ...
        for study_dir, current_timestamp in study_dirs:
            ...
            if not study_needs_update(
                study_path, current_timestamp, cached_study
            ):
            ...
            current_json_files = get_json_files_from_study(
                OAS_BASE_URL, study_path
            )
            ...
            if not cached_study:
                ...
                stats["studies_new"] += 1
                logger.info(
                    f"    New study - parsing all {len(current_json_files)} JSON files"
                )
                ...
                logger.info(
                    f"    Files: {len(new_files)} new, {len(changed_files)} changed, {len(unchanged_files)} unchanged"
                )
    ...
    total_json_files = sum(
        len(study.json_files) for study in updated_index.values()
    )
    total_with_metadata = sum(
        len(study.metadata) for study in updated_index.values()
    )
...
def determine_indexing_scope(filters: Dict[str, str]) -> List[str]:
    ...
    if chain_filter:
        ...
        has_paired = any(
            indicator in chain_values for indicator in paired_indicators
        )
        has_unpaired = any(
            indicator in chain_values for indicator in unpaired_indicators
        )
...
def build_complete_json_index(
    ...
    logger.info(
        f"Building JSON index from OAS server for directories: {', '.join(directories)}"
    )
    ...
    logger.info("Complete JSON index built!")
    logger.info(f"  Total studies indexed: {len(complete_index)}")

    total_json_files = sum(
        len(study.json_files) for study in complete_index.values()
    )
    total_with_metadata = sum(
        len(study.metadata) for study in complete_index.values()
    )
        ...
        try:
            with open(index_file, "rb") as f:
                ...
                if isinstance(index_data, dict) and "index" in index_data:
                    ...
                    metadata = {
                        k: v for k, v in index_data.items() if k != "index"
                    }
                    return metadata
...
def save_json_index(index: Dict[str, StudyInfo]):
    ...
    try:
        ...
        index_data = {
            ...
            "total_json_files": sum(
                len(study.json_files) for study in index.values()
            ),
            "total_with_metadata": sum(
                len(study.metadata) for study in index.values()
            ),
        }
        ...
        for study in index.values():
            for meta in study.metadata.values():
                if (
                    "Total sequences" in meta
                    and meta["Total sequences"] is not None
                ):
                    try:
        ...
        logger.info(
            f"  {len(index)} studies, {index_data['total_json_files']} JSON files, {total_sequences:,} total sequences indexed"
        )
...
def study_needs_update(
    study_path: str, current_timestamp: str, cached_study: StudyInfo = None
) -> bool:
    """
    ...
    if current_timestamp != cached_study.last_modified:
        logger.debug(
            f"  Modified study: {study_path} ({cached_study.last_modified} -> {current_timestamp})"
        )
        return True
...
def get_changed_files(
    current_files: List[FileInfo], cached_files: List[FileInfo]
) -> tuple:
    """
...
def validate_search_criteria(
    index: Dict[str, StudyInfo], filters: Dict[str, str]
) -> List[str]:
    """
            ...
            if invalid_values:
                ...
                for invalid_val in invalid_values:
                    ...
                    for val in available_values:
                        if (
                            invalid_lower in str(val).lower()
                            or str(val).lower() in invalid_lower
                        ):
                            closest_matches.append(val)

                error_msg = (
                    f"No studies found with {filter_key}='{filter_value}'"
                )
                if invalid_values:
                    error_msg += (
                ...
                if closest_matches:
                    ...
                    if len(available_values) > 10:
                        error_msg += (
                            f" (and {len(available_values) - 10} more...)"
                        )
...
def matches_filter_criteria(metadata: Dict, filters: Dict[str, str]) -> bool:
    ...
    for filter_key, filter_value in filters.items():
        ...
        if filter_key in metadata:
            ...
            # Split filter value by spaces to handle multiple selections
            filter_values = [
                v.strip().lower() for v in str(filter_value).split()
            ]
...
def get_csv_files_for_study(
    study_path: str, study_info, base_url: str
) -> List[Dict]:
    """
...
def calculate_total_size_for_criteria(
    index: Dict, filters: Dict[str, str], base_url: str
) -> Dict:
    """
    ...
    for study_path, study_info in index.items():
        ...
        try:
            csv_files = get_csv_files_for_study(
                study_path, study_info, base_url
            )
            ...
            for json_filename, csv_filename, metadata in files_to_check:
                if csv_filename in csv_lookup:
                    ...
                    logger.debug(
                        f"  ✓ {csv_filename}: {file_size / (1024 * 1024):.1f} MB"
                    )
                else:
    ...
    logger.info("File size calculation complete:")
...
def filter_and_display_matches(
    index: Dict[str, StudyInfo], filters: Dict[str, str], verbose: bool = False
):
    """
    ...
    for study_path, study_info in index.items():
        ...
        if matching_files:
            ...
            for json_filename, metadata in matching_files:
                ...
                has_unique = (
                    "Unique sequences" in metadata
                    and metadata["Unique sequences"] is not None
                )
                if has_unique:
                    files_with_unique += 1
                    try:
                        ...
                        if isinstance(unique_seq, (int, float)):
                            ...
                            isinstance(unique_seq, str) and unique_seq.isdigit()
                        ):
                            study_sequences += int(unique_seq)
                ...
                has_total = (
                    "Total sequences" in metadata
                    and metadata["Total sequences"] is not None
                )
                if has_total:
                    files_with_total += 1
                    try:
    ...
    size_results = calculate_total_size_for_criteria(
        index, filters, OAS_BASE_URL
    )
    ...
    if files_missing_total > 0:
        missing_percentage = (
            (files_missing_total / total_matching_files * 100)
            if total_matching_files > 0
            else 0
        )
        logger.warning(
            f"WARNING: {files_missing_total} out of {total_matching_files} matching files ({missing_percentage:.1f}%) are missing the 'Total sequences' value."
        )
        logger.warning(
            f"   The 'Total sequences' sum shown above ({total_matching_total_sequences:,}) is INCOMPLETE"
        )
        logger.warning(
            f"   and does not include sequences from these {files_missing_total} files."
        )
    ...
    logger.info(
        f"  Total file size: {size_results['total_size_gb']:.2f} GB ({size_results['total_size_mb']:.1f} MB)"
    )

    if matching_studies:
        if verbose:
            logger.info("\n📋 Matching Studies and JSON Files:")
            for i, (study_path, study_info, matching_files) in enumerate(
                matching_studies
            ):
                logger.info(f"  {i + 1}. {study_path}")
                logger.info(
                    f"     Type: {'Paired' if study_info.is_paired else 'Unpaired'}"
                )
                logger.info(
                    f"     Total JSON files: {len(study_info.json_files)}"
                )
                logger.info(f"     Matching files: {len(matching_files)}")
                ...
                for j, (json_filename, metadata) in enumerate(matching_files):
                    ...
                    logger.info(
                        f"          Species: {metadata.get('Species', 'N/A')}"
                    )
                    logger.info(
                        f"          Disease: {metadata.get('Disease', 'N/A')}"
                    )
                    logger.info(
                        f"          Vaccine: {metadata.get('Vaccine', 'N/A')}"
                    )
                    logger.info(
                        f"          Chain: {metadata.get('Chain', 'N/A')}"
                    )
                    logger.info(
                        f"          Isotype: {metadata.get('Isotype', 'N/A')}"
                    )
                    logger.info(
            ...
            # Show summary only
            logger.info("Matching Studies Summary:")
            for i, (study_path, study_info, matching_files) in enumerate(
                matching_studies
            ):
                logger.info(
                    f"  {i + 1}. {study_path} ({'Paired' if study_info.is_paired else 'Unpaired'}) - {len(matching_files)} matching files"
                )
            logger.info("Use --verbose to see detailed file information")
    else:
...
def preview_download_plan(matching_studies: List, output_dir: Path):
    ...
    total_files = (
        len(file_status["existing"])
        + len(file_status["new"])
        + len(file_status["updated"])
    )
    ...
    if file_status["new"]:
        ...
        for file_info in file_status["new"][:10]:  # Show first 10
            ...
            csv_filename = file_info["json_filename"].replace(
                ".json", ".csv.gz"
            )
            ...
            if study_info:
                try:
                    csv_files = get_csv_files_for_study(
                        study_path, study_info, OAS_BASE_URL
                    )
                    csv_lookup = {f["name"]: f for f in csv_files}

                    if csv_filename in csv_lookup:
                        file_size_mb = csv_lookup[csv_filename][
                            "size_bytes"
                        ] / (1024 * 1024)
                        total_download_size_mb += file_size_mb
                        logger.info(
                            f"  • {file_info['parquet_filename']}: {file_size_mb:.1f} MB ({file_info['metadata'].get('Species', 'N/A')})"
                        )
                    else:
                        logger.info(
                            f"  • {file_info['parquet_filename']}: Size unknown ({file_info['metadata'].get('Species', 'N/A')})"
                        )
                except Exception:
                    logger.info(
                        f"  • {file_info['parquet_filename']}: Size unknown ({file_info['metadata'].get('Species', 'N/A')})"
    ...
    if file_status["existing"]:
        ...
        for file_info in file_status["existing"][:5]:  # Show first 5
            logger.info(
                f"  • {file_info['parquet_filename']} ({file_info['metadata'].get('Species', 'N/A')})"
            )
...
def download_and_convert_study_files(
    ...
    if max_files is not None:
        matching_studies, total_before, total_after = limit_matching_studies(
            matching_studies, max_files
        )
        logger.info(
            f"File limit applied: {max_files} files (out of {total_before} matching files)"
        )
        ...
        for json_filename, metadata in matching_files:
            ...
            for csv_url in csv_urls:
                try:
                    ...
                    stats = convert_file(
                        temp_csv_path, output_dir, extraction_level=1
                    )
                    ...
                    if "error" not in stats:
                        ...
                        logger.info(
                            f"    Converted successfully: {stats['rows']} rows, {csv_size_mb:.1f}MB → {parquet_size_mb:.1f}MB ({compression_ratio:.1f}x compression)"
                        )
                        downloaded = True
            ...
            if not downloaded:
                logger.error(
                    f"    Failed to download {csv_filename} from any URL"
                )
    ...
    if successful_conversions > 0:
        overall_compression = (
            total_downloaded_mb / total_parquet_mb
            if total_parquet_mb > 0
            else 1.0
        )
        logger.info("\nCompression Summary:")
        logger.info(
            f"  Total downloaded (CSV.gz): {total_downloaded_mb:.1f} MB"
        )
        ...
        logger.info(
            f"  Space saved: {total_downloaded_mb - total_parquet_mb:.1f} MB ({(1 - total_parquet_mb / total_downloaded_mb) * 100:.1f}%)"
        )

    logger.info(
        f"Processing complete! Processed {total_files} files, {successful_conversions} successful conversions, {skipped_files} files already existed"
    )
...
def inspect_index():
    ...
    if index_metadata and "built_at" in index_metadata:
        ...
        try:
            ...
            print(
                f"  Last updated: {built_at.strftime('%Y-%m-%d %H:%M:%S')} ({age_str} ago)"
            )

            # Always warn that index may not be up to date (since --inspect doesn't check server)
            if age.days > 0:
                print(
                    f"  WARNING: Index may not be up to date (last updated {age_str} ago)!"
                )
            else:
                print("  WARNING: Index may not be up to date!")
            print(
                "     Run the script without --inspect to check for updates on the OAS server."
            )
        except (ValueError, TypeError):
            print(f"  Last updated: {built_at_str} (could not parse timestamp)")
    else:
        print(
            "  WARNING: Index metadata not available (may be from older version)"
        )
        print("     Index may not be up to date!")
        print(
            "     Run the script without --inspect to update the index and check for new files."
        )
...
def main():
    ...
    logger.info("=" * 60)
    logger.info(
        "ABHunter - Update from OAS script: Download and update local databse from the Observed Antibody Space (OAS) database"
    )
    ...
    logger.info(
        f"Selective indexing: {', '.join(indexing_directories)} directories"
    )
    ...
    if args.rebuild:
        ...
        if existing_index:
            ...
            logger.info(
                "No existing index found, building index for selected directories..."
            )
            complete_index = build_complete_json_index(indexing_directories)
    ...
    matching_studies = filter_and_display_matches(
        complete_index, filters, args.verbose
    )

    if matching_studies:
        logger.info(
            f"Found {len(matching_studies)} studies with matching criteria"
        )
        logger.info(
            "These studies contain JSON files that match your filter criteria"
        )
        ...
        if args.preview:
            ...
            if args.max_files is not None:
                matching_studies, total_before, total_after = (
                    limit_matching_studies(matching_studies, args.max_files)
                )
                logger.info(
                    f"File limit applied: {args.max_files} files (out of {total_before} matching files)"
                )
                if total_before > total_after:
                    logger.info(
                        f"   Preview shows first {total_after} files only"
                    )
            ...
            try:
                ...
                success = download_and_convert_study_files(
                    matching_studies,
                    tmp_dir,
                    output_dir,
                    args.keep_csv,
                ...
                if success:
                    ...
                    if args.keep_csv:
                        ...
                        logger.info("Files processed with immediate cleanup")
                else:
                ...
                if not args.keep_csv and tmp_dir.exists():
                    ...
                    try:
                        ...
                        logger.info(
                            f"Cleaned up temporary directory: {tmp_dir}"
                        )
                    except Exception as e:
                        logger.warning(
                            f"Could not clean up temp directory: {e}"
                        )
                elif args.keep_csv:
                    logger.info(f"Temporary directory preserved: {tmp_dir}")
        else:
            logger.info(
                "Next step: Use --preview to see what would be downloaded, or --download-and-convert to download and convert matching CSV.gz files to Parquet"
            )
    else:
        logger.info("\nNo studies match the specified criteria")
        logger.info(
            "Try adjusting your filter criteria or use --inspect to see available options"
        )
